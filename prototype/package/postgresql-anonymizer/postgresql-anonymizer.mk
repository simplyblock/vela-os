POSTGRESQL_ANONYMIZER_VERSION = 08042ff527655ba00f8db863830215452e817a2d
POSTGRESQL_ANONYMIZER_SITE = $(call gitlab,dalibo,postgresql_anonymizer,$(POSTGRESQL_ANONYMIZER_VERSION))
POSTGRESQL_ANONYMIZER_LICENSE = Apache-2.0
POSTGRESQL_ANONYMIZER_LICENSE_FILES = LICENSE

POSTGRESQL_ANONYMIZER_DEPENDENCIES = postgresql host-cargo-pgrx
POSTGRESQL_ANONYMIZER_CARGO_TARGET_ENV = $(call UPPERCASE,$(subst -,_,$(RUSTC_TARGET_NAME)))

POSTGRESQL_ANONYMIZER_CARGO_ENV = \
	PGRX_PG_CONFIG_PATH="$(POSTGRESQL_ANONYMIZER_PG_CONFIG_WRAPPER)" \
...
POSTGRESQL_ANONYMIZER_CARGO_BUILD_OPTS += --features pg18
POSTGRESQL_ANONYMIZER_CARGO_INSTALL_OPTS =
POSTGRESQL_ANONYMIZER_CARGO_INSTALL_TARGET_OPTS =

POSTGRESQL_ANONYMIZER_PGRX_OUTDIR = $(@D)/pgrx-out
POSTGRESQL_ANONYMIZER_PG_CONFIG_WRAPPER = $(@D)/pg_config.br

define POSTGRESQL_ANONYMIZER_CREATE_PG_CONFIG_WRAPPER
	{ \
	  echo '#!/bin/sh'; \
	  ...
	  echo '  *) echo "";;'; \
	  echo 'esac'; \
	} >"$(POSTGRESQL_ANONYMIZER_PG_CONFIG_WRAPPER)"
	chmod +x "$(POSTGRESQL_ANONYMIZER_PG_CONFIG_WRAPPER)"
endef

POSTGRESQL_ANONYMIZER_PRE_BUILD_HOOKS += POSTGRESQL_ANONYMIZER_CREATE_PG_CONFIG_WRAPPER

define POSTGRESQL_ANONYMIZER_GENERATE_PGRX_PACKAGE
	rm -rf $(POSTGRESQL_ANONYMIZER_PGRX_OUTDIR)
	mkdir -p $(POSTGRESQL_ANONYMIZER_PGRX_OUTDIR) $(@D)/.pgrx
	...
	env -u CC -u CXX -u AR -u RANLIB -u CFLAGS -u CXXFLAGS -u LDFLAGS \
		...
		CARGO_TARGET_$(POSTGRESQL_ANONYMIZER_CARGO_TARGET_ENV)_LINKER="$(TARGET_CC)" \
		CARGO_TARGET_$(POSTGRESQL_ANONYMIZER_CARGO_TARGET_ENV)_AR="$(TARGET_AR)" \
		$(HOST_DIR)/bin/cargo-pgrx pgrx package \
			--no-default-features \
			--features "pg18" \
			--pg-config "$(POSTGRESQL_ANONYMIZER_PG_CONFIG_WRAPPER)" \
			--out-dir "$(POSTGRESQL_ANONYMIZER_PGRX_OUTDIR)" \
			--manifest-path "$(@D)/Cargo.toml" \
			--target "$(RUSTC_TARGET_NAME)"
endef

POSTGRESQL_ANONYMIZER_POST_BUILD_HOOKS += POSTGRESQL_ANONYMIZER_GENERATE_PGRX_PACKAGE

define POSTGRESQL_ANONYMIZER_INSTALL_TARGET_CMDS
	# Prefer the cargo-pgrx package output, which contains the generated SQL. :contentReference[oaicite:4]{index=4}
	PG_PKG="$(POSTGRESQL_ANONYMIZER_PGRX_OUTDIR)"; \
	if [ ! -d "$$PG_PKG" ]; then \
		echo "ERROR: pgrx out dir missing: $$PG_PKG"; \
		exit 1; \
	fi; \
	\
	SO="$$(find "$$PG_PKG" -type f -name 'anon*.so' | head -n1)"; \
	CTRL="$$(find "$$PG_PKG" -type f -name 'anon.control' | head -n1)"; \
	SQLS="$$(find "$$PG_PKG" -type f -name 'anon--*.sql' | tr '\n' ' ')"; \
	\
	...
	echo "Installing $$CTRL to $(TARGET_DIR)/usr/share/postgresql/extension/anon.control..."; \
	install -m 0644 "$$CTRL" "$(TARGET_DIR)/usr/share/postgresql/extension/anon.control"; \
	for f in $$SQLS; do \
...
define POSTGRESQL_ANONYMIZER_INSTALL_STAGING_CMDS
	:
endef
