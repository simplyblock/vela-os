...
PG_JSONSCHEMA_VERSION = 7c8603f14d8d20ea84435b0b8409a4e1a40147b0
PG_JSONSCHEMA_SITE = $(call github,supabase,pg_jsonschema,$(PG_JSONSCHEMA_VERSION))
PG_JSONSCHEMA_LICENSE = Apache-2.0
PG_JSONSCHEMA_LICENSE_FILES = LICENSE

PG_JSONSCHEMA_DEPENDENCIES = postgresql host-cargo-pgrx
PG_JSONSCHEMA_CARGO_TARGET_ENV = $(call UPPERCASE,$(subst -,_,$(RUSTC_TARGET_NAME)))

PG_JSONSCHEMA_CARGO_ENV = \
	PGRX_PG_CONFIG_PATH="$(PG_JSONSCHEMA_PG_CONFIG_WRAPPER)" \
...
#PG_JSONSCHEMA_CARGO_BUILD_OPTS += --features pg18
PG_JSONSCHEMA_CARGO_INSTALL_OPTS =
PG_JSONSCHEMA_CARGO_INSTALL_TARGET_OPTS =

PG_JSONSCHEMA_PGRX_OUTDIR = $(@D)/pgrx-out
PG_JSONSCHEMA_PG_CONFIG_WRAPPER = $(@D)/pg_config.br

define PG_JSONSCHEMA_CREATE_PG_CONFIG_WRAPPER
	{ \
	  echo '#!/bin/sh'; \
	  ...
	  echo '  *) echo "";;'; \
	  echo 'esac'; \
	} >"$(PG_JSONSCHEMA_PG_CONFIG_WRAPPER)"
	chmod +x "$(PG_JSONSCHEMA_PG_CONFIG_WRAPPER)"
endef

PG_JSONSCHEMA_PRE_BUILD_HOOKS += PG_JSONSCHEMA_CREATE_PG_CONFIG_WRAPPER

define PG_JSONSCHEMA_GENERATE_PGRX_PACKAGE
	rm -rf $(PG_JSONSCHEMA_PGRX_OUTDIR)
	mkdir -p $(PG_JSONSCHEMA_PGRX_OUTDIR) $(@D)/.pgrx
	...
	env -u CC -u CXX -u AR -u RANLIB -u CFLAGS -u CXXFLAGS -u LDFLAGS \
		...
		CARGO_TARGET_$(PG_JSONSCHEMA_CARGO_TARGET_ENV)_LINKER="$(TARGET_CC)" \
		CARGO_TARGET_$(PG_JSONSCHEMA_CARGO_TARGET_ENV)_AR="$(TARGET_AR)" \
		$(HOST_DIR)/bin/cargo-pgrx pgrx package \
			--no-default-features \
			--features "pg18" \
			--pg-config "$(PG_JSONSCHEMA_PG_CONFIG_WRAPPER)" \
			--out-dir "$(PG_JSONSCHEMA_PGRX_OUTDIR)" \
			--manifest-path "$(@D)/Cargo.toml" \
			--target "$(RUSTC_TARGET_NAME)"
endef

PG_JSONSCHEMA_POST_BUILD_HOOKS += PG_JSONSCHEMA_GENERATE_PGRX_PACKAGE

define PG_JSONSCHEMA_INSTALL_TARGET_CMDS
	# Prefer the cargo-pgrx package output, which contains the generated SQL. :contentReference[oaicite:4]{index=4}
	PG_PKG="$(PG_JSONSCHEMA_PGRX_OUTDIR)"; \
	if [ ! -d "$$PG_PKG" ]; then \
		echo "ERROR: pgrx out dir missing: $$PG_PKG"; \
		exit 1; \
	fi; \
	\
	SO="$$(find "$$PG_PKG" -type f -name 'pg_jsonschema*.so' | head -n1)"; \
	CTRL="$$(find "$$PG_PKG" -type f -name 'pg_jsonschema.control' | head -n1)"; \
	SQLS="$$(find "$$PG_PKG" -type f -name 'pg_jsonschema--*.sql' | tr '\n' ' ')"; \
	\
	...
	echo "Installing $$CTRL to $(TARGET_DIR)/usr/share/postgresql/extension/pg_jsonschema.control..."; \
	install -m 0644 "$$CTRL" "$(TARGET_DIR)/usr/share/postgresql/extension/pg_jsonschema.control"; \
	for f in $$SQLS; do \
...
define PG_JSONSCHEMA_INSTALL_STAGING_CMDS
	:
endef
